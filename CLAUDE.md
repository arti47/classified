...
**The guided player.** `coach.js`, on the Play screen and at the top of Solo. The play guide
tells a player what to do; somebody who will not read a manual will not read a guide either, so
the coach *does it with them*: one instruction, one button, recomputed after every action —
no character → **Give me a ready-made agent**; no mission → **Start a mission**, which turns
solo on, opens the adventure and rolls the whole briefing in that one tap, then reads it back
as *what you are after / what is in the way / who you are pretending to be*; then a field
asking what you are about to do, and inside the scene the only question that matters, in three
plain choices — **my agent tries something**, **I need to know if something is true**, **I am
stuck, give me something** — which are the skill roller, Fate at odds worded as *probably /
could go either way / probably not*, and a Meaning Table. It owns no rule: every beat is a
dynamic import into the engine that already exists, so the coach can be deleted without taking
a procedure with it (ruling N12).

**The play guide.** `#/play`, from a Home tile and the first-run card. The tutorial narrates a
mission somebody else played, the how-to panels explain the panel in front of you, and the
first-run card names three things and then disappears the moment play begins — so none of them
answered *what do I do next in my own game*, which is the only question a new player has. The
guide answers it for the whole arc: **Start a game**, **Keep it going**, **End it well**. Its
steps tick themselves off against live state, the next undone one is pulled to the top with the
control that does it, and the middle act is a five-step loop rather than a checklist, because
that is what a scene is. Two tracks — solo and table — because the two games end differently:
the solo player closes the mission from the Solo screen, the table's referee calls it and End
Mission is on Combat (ruling N11).
...
| N12 | The play guide answered *what do I do next* — in writing. The player who needed it said, twice, that they still could not start a game, sustain one, or end it: reading a guide is still reading, and the answer to "how do I play" cannot be a document | The **coach**: the app plays it with you. One instruction, one button, recomputed after every action, from an empty device to a closed mission. It makes the agent, turns solo on, opens the adventure, rolls the briefing and reads it back in plain words, asks what you are about to do, and inside a scene reduces the whole system to three choices in the player's own language. Nothing to read, nothing to find, and no jargon in any beat | A whole mission driven from the card alone on an empty device: agent made, solo switched on, briefing rolled and read back, scene opened, three choices offered, scene closed and the counter moved |
| N11 | Nothing told a player how to **play**: how to start a game, how to sustain one, or how to end it well. The tutorial is a mission somebody else played, in the past tense; the how-to panels explain one panel each; the first-run card names three things and vanishes the moment play begins. A player who had built a character still did not know what to do next | A **play guide** at `#/play`, from a Home tile and the first-run card: three acts — start, sustain, end — whose steps tick themselves off against live state, with the next undone one pulled to the top and a button that does it. The sustaining act is the five-step scene loop rather than a checklist. Two tracks, solo and table, because the two games end differently | Both tracks render their own loop and ending, the acts are the three a game has, a done step is ticked rather than asked for again, and the next step moves on as the game does |
...
| 2026-08-19 | Added the play guide (N11): `#/play`, a Home tile, and the first-run card's middle step. Three acts — start a game, keep it going, end it well — ticking themselves off against the live dossier and adventure, with one next step pulled to the top and the scene loop written out as a loop | Reported, and the most important thing anyone has said about this app: after all of it, the player still did not know how to start playing, sustain play, or end a game well. Every existing surface explains a screen; none explains a *game*. The tutorial is the worst offender — it is a story about somebody else's mission, so it teaches by anecdote and cannot say what to do now. The guide is stateful for exactly that reason: it always knows which of the three acts you are in and what the single next tap is | 1265 checks green over three consecutive sweeps, including both tracks rendering their own loop and their own ending, the three acts in order, a done step ticked rather than re-asked, and the next step moving on when an adventure is opened | `classified-v32` |
| 2026-08-19 | Added the guided player (N12): `src/coach.js`, on the Play screen and the top of Solo. One instruction and one button at a time, from an empty device to a closed mission — including the three-choice in-scene router that is the whole system in the player's own words. `autoBriefing()` and an optional expectation on `startScene()` let it drive the Mythic layer without opening a form | Reported twice, in the same words both times: still did not know how to start playing, sustain play, or end a game. The first answer was a written guide, which was the same mistake one level down — a player who will not read a manual will not read a guide. This one does not explain the game, it runs it, and every beat is a dynamic import into an engine that already existed, so no rule moved into the conductor | 1289 checks green over three consecutive sweeps, including a whole mission driven from the coach card alone on a device with no dossier and solo switched off | `classified-v33` |
